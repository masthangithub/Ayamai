...
def generate_narrative(entropy_text):
    """
        ...
        Generates poetic, dream-like narrative using Gemini API and extracts first option if present.
    """
    output = gemini_api_generate(entropy_text)
    output = extract_first_option(output)
    return output

def extract_first_option(text):
    """
    Extracts the first option from a Gemini response if options are present.
    """
    import re
    # Look for 'Option 1:' or '1.' or similar patterns
    option_pattern = re.compile(r'(Option 1:|1\.|1\))', re.IGNORECASE)
    match = option_pattern.search(text)
    if match:
        # Split by 'Option' or number, then take the first non-empty chunk
        parts = re.split(r'Option \d:|\n\d[\.)]', text, flags=re.IGNORECASE)
        for part in parts:
            cleaned = part.strip()
            if cleaned:
                return cleaned
...
def gemini_api_generate(prompt, api_key=GOOGLE_API_KEY, max_tokens=100, model_name="gemini-1.5-flash-latest"):
...
def dreaming_ai_core(input_data, goal, iterations=3, use_rag=True):
    ...
    for i in range(iterations):
        ...
        if use_rag and web_content:
            ...
            prompt = f"{current_input}\nGoal: {goal}"
        llm_output = gemini_api_generate(prompt, max_tokens=100)
...
if __name__ == '__main__':
    ...
    goal = "Trains revolutionize the steel industry "
